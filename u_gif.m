...
% 
%  u_gif('filename.gif') 
%  u_gif(...,'DelayTime',DelayTimeValue,...) 
%  u_gif(...,'LoopCount',LoopCountValue,...) 
%  u_gif(...,'frame',handle,...) 
%  u_gif(...,'resolution',res)
%  u_gif(...,'nodither') 
%  u_gif(...,'overwrite',true)
%  u_gif 
%  u_gif('clear') 
% 
%% Description 
% 
% u_gif('filename.gif') writes the first frame of a new gif file by the name filename.gif. 
% 
% u_gif(...,'DelayTime',DelayTimeValue,...) specifies a the delay time in seconds between
% frames. Default delay time is 1/15. 
% 
% u_gif(...,'LoopCount',LoopCountValue,...) specifies the number of times the gif animation 
% will play. Default loop count is Inf. 
% 
% u_gif(...,'frame',handle,...) uses the frame of the given figure or set of axes. The default 
...
% gca to gcf.
% 
% u_gif(...,'resolution',res) specifies the resolution (in dpi) of each frame. This option
% requires export_fig (https://www.mathworks.com/matlabcentral/fileexchange/23629).
%
% u_gif(...,'nodither') maps each color in the original image to the closest color in the new 
...
% 
% u_gif(...,'overwrite',true) bypasses a dialoge box that would otherwise verify 
% that you want to overwrite an existing file by the specified name. 
%
% u_gif adds a frame to the current gif file. 
% 
% u_gif('clear') clears the persistent variables associated with the most recent gif. 
% 
%% Example 
% For examples, type 
% 
